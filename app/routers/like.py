...
def like(likes: schemas.Like, db: Session = Depends(config.get_db), current_user: int = Depends(oauth2.get_current_user)):

    post = db.query(models.Post).filter(
        models.Post.id == likes.post_id).first()  # checking for post
    if post is None:
        raise HTTPException(
            status_code=404,
            detail=f'Post with id {likes.post_id} does not exist'
        )
    ...
    if (likes.dir == 1):
        if found_like:
            raise HTTPException(
                status_code=409,
                detail=f'user {current_user.id} already liked post'
            )
