...
async def delete_post(
    ...
    if post.owner_id != current_user.id:
        raise HTTPException(
            status_code=401, detail="User Not Authorized to perform requested action"
        )
...
async def update_post(
    ...
    if post is None:
        raise HTTPException(
            status_code=404, detail=f"Post with id {id} does not exists"
        )
    if post.owner_id != current_user.id:
        raise HTTPException(
            status_code=401, detail="User Not Authorized to perform requested action"
    ...
    post_query.filter(Post.id == id).update(update_data, synchronize_session=False)
    db.commit()
    db.refresh(post)
    return post
