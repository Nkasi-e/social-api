...
# Using a pydantic model - request can be made from raw JSON on postman

# @router.post('/login')
# async def login(user_credencials: UserLogin, db: Session = Depends(get_db)):
#     user = db.query(User).filter(User.email == user_credencials.email).first()
#     if not user:
#         raise HTTPException(
#             status_code=404,
#             detail='Invalid credentials'
